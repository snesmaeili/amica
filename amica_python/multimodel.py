...
def _model_accumulate(y_h, v_h, alpha_h, mu_h, beta_h, rho_h):
    ...
    return (gy, u_sum, mu_n, mu_d_le2, mu_d_gt2, beta_d_le2, beta_d_gt2,
            rho_n, kappa_n, lambda_n)


def _model_posteriors_from_data(data_white, W_all, c_all, alpha_all, mu_all,
                                beta_all, rho_all, gm, log_det_sphere):
...
def m_step_mm(
    ...
    def per_model(gy_h, sig_h, ds_h, Nv_h, rs_h, mn_h, mdle_h, mdgt_h,
                  bdle_h, bdgt_h, rn_h, kn_h, ln_h,
                  W_h, A_h, c_h, alpha_h, mu_h, beta_h, rho_h):
    ...
    (W_new, A_new, c_new, alpha_new, mu_new, beta_new, rho_new,
     is_good_all, newton_all) = jax.vmap(per_model)(
        totals.gy_partial, totals.sigma2_partial, totals.data_sum, totals.Nv,
        totals.resp_sum, totals.mu_numer, totals.mu_denom_le2, totals.mu_denom_gt2,
        totals.beta_denom_le2, totals.beta_denom_gt2, totals.rho_numer,
        totals.kappa_numer, totals.lambda_numer,
        W_all, A_all, c_all, alpha_all, mu_all, beta_all, rho_all,
    )
    ...
    return (W_new, A_new, c_new, alpha_new, mu_new, beta_new, rho_new, gm_new,
            ll, is_good, newton_used)


def compute_model_posteriors(
    data_white, W_all, c_all, alpha_all, mu_all, beta_all, rho_all, gm,
