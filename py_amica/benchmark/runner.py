...
def estimate_events_to_merged_offset(raw, rows):
    ...
    tsv_m1 = [
        float(row["_onset"]) for row in rows if normalize_event_label(row.get("value", "")) == "M 1"
    ]
...
def ds004505_event_metadata(raw, bids_root, subject_id):
    ...
    metadata = {
        ...
        "events_to_merged_offset_s": estimate_events_to_merged_offset(raw, rows) if rows else None,
    }
...
def select_ds004505_scalp_eeg(raw, set_path=None):
    ...
    scalp_chs = [ch for ch in groups["scalp_eeg"] if raw.get_channel_types(picks=[ch])[0] == "eeg"]
    if not scalp_chs:
...
def build_input_metadata(raw, input_metadata=None):
    ...
    info_proj = (
        raw.info.get("custom_ref_applied")
        if isinstance(raw.info, dict)
        else getattr(raw.info, "custom_ref_applied", None)
    )
    ...
    metadata.setdefault(
        ...
        list(
            raw.info.get("bads", [])
            if isinstance(raw.info, dict)
            else getattr(raw.info, "bads", [])
        ),
    )
    metadata.setdefault("annotations_excluded", [])
    metadata["n_channels_ica"] = int(len(raw.ch_names))
    metadata.setdefault(
        "n_channels_input", int(metadata.get("n_loaded_channels", len(raw.ch_names)))
    )
...
def output_filename(
    dataset, subject, backend, device, schema_version="legacy", hp_freq=DEFAULT_HP_FREQ
):
...
def channel_positions_3d(raw):
    ...
    for idx, _name in enumerate(raw.ch_names):
        ...
        if idx < len(chs):
            loc = (
                chs[idx].get("loc")
                if isinstance(chs[idx], dict)
                else getattr(chs[idx], "loc", None)
            )
        if loc is None:
...
def build_v3_document(
    *,
    ...
    kappa_channels = (
        float(n_samples_used) / float(n_channels_used**2) if n_channels_used > 0 else None
    )
    kappa_effective = (
        float(n_samples_used) / float(n_components_used**2) if n_components_used > 0 else None
    )
    data = {
        "dataset": dataset,
...
def load_data(dataset_name, subject_id, task=None, input_level="auto", return_metadata=False):
    ...
    if dataset_name == "mne":
        ...
        target_set_file = find_ds004505_set_file(bids_root, subject_id, input_level=input_level)
...
def compute_v3_artifacts(ica, raw):
    ...
    try:
        ...
        remnant_pct = (
            100.0 * src_mean / scalp_mean
            if scalp_mean and np.isfinite(scalp_mean) and scalp_mean > 0
            else float("nan")
        )
        artifacts["pmi"] = {
    ...
    try:
        ...
        artifacts["reconstruction_error"] = float(
            np.linalg.norm(data - recon) / np.linalg.norm(data)
        )
    except Exception as exc:
...
def reaggregate_from_ica(json_path, ica_path, raw, *, out_path=None, preserve_keys=None):
...
def run_benchmark(
    ...
    *,
    ...
    requires an upstream callback hook in ``py_amica.solver.Amica.fit``
    ...
    # Force reload of py_amica.backend to apply env vars
    import importlib
    ...
    if do_reject:
        fit_params.update(
            do_reject=True,
            rejsig=float(rejsig),
            rejstart=int(rejstart),
            rejint=int(rejint),
            numrej=int(numrej),
        )

    mem = _measure_peak_memory(
        backend,
        device,
        fit_ica,
        raw,
    ...
    converged_amica_flag = (
        bool(getattr(amica_result_obj, "converged", False))
        if amica_result_obj is not None
        else False
    )
    metrics = {
        "method": "amica",
    ...
    _iter_times = np.asarray(getattr(amica_result_obj, "iteration_times", []), dtype=float)
    ...
    try:
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=RuntimeWarning, message=".*ICLabel.*")
            labels = label_components(raw, ica, method="iclabel")
        for label, prob in zip(labels["labels"], labels["y_pred_proba"], strict=False):
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="mne", choices=["mne", "ds004505"])
    parser.add_argument("--device", type=str, choices=["cpu", "gpu"], default="cpu")
    parser.add_argument("--backend", type=str, choices=["jax", "numpy"], default="jax")
    parser.add_argument(
        "--task", type=str, default=None, help="BIDS task name (auto-detected if omitted)"
    )
    ...
    print(
        f"Processing {args.dataset} Subject {args.subject} | "
