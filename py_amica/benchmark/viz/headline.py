...
def iclabel_panel(ax, methods: dict):
    ...
    for cls in ICLABEL_CLASSES:
        ax.bar(
            ...
            width=width,
            label=cls,
            color=ICLABEL_COLORS[cls],
        )
        bottom += np.asarray(composition[cls])
    ax.set_xticks(range(len(valid)))
    ax.set_xticklabels(
        [METHOD_LABELS.get(k, k) for k in valid], rotation=30, ha="right", fontsize=8
    )
...
def convergence_panel(ax, methods: dict):
    ...
    if not plotted:
        ax.text(
            0.5,
            0.5,
            "No convergence trace available\n(only AMICA-Python records LL)",
            ha="center",
            va="center",
            fontsize=9,
        )
...
def main():
    ...
    fig.suptitle(
        f"AMICA-Python vs comparators ({subject_label}, ds004505 TableTennis)", fontsize=12
    )
