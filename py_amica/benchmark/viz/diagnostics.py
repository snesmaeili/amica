...
def read_bids_events(bids_root: Path, subject: int) -> tuple[Path | None, list[dict]]:
    event_file = (
        bids_root / f"sub-{subject:02d}" / "eeg" / f"sub-{subject:02d}_task-TableTennis_events.tsv"
    )
...
def estimate_events_to_merged_offset(raw, rows: list[dict]) -> float | None:
    ...
    tsv_m1 = [
        float(row["_onset"]) for row in rows if normalize_marker(row.get("value", "")) == "M 1"
    ]
...
def plot_convergence_runtime(
    out_dir: Path, result, metrics: dict, runtime_rows: list[dict]
) -> None:
    ...
    fig.suptitle(
        (f"AMICA-Python sub-04: 64 ICs, 20 iterations, {metrics['runtime_s']:.1f} s total"),
        fontsize=10,
        fontweight="bold",
        y=0.99,
    )
...
def plot_iclabel_composition(out_dir: Path, labels: list[str], probs: np.ndarray) -> None:
    ...
    left = 0.0
    for frac, (_name, color) in zip(fractions, ICLABEL_COLORS.items(), strict=False):
    ...
    handles = [
        plt.Line2D(
            [0], [0], marker="s", color="none", markerfacecolor=color, markersize=6, label=name
        )
...
def plot_component_examples(
    out_dir: Path, raw, ica, labels: list[str], probs: np.ndarray, result
) -> None:
    ...
    data = [
        [rho_weighted[i] for i, label in enumerate(labels) if label == cls] for cls in class_order
    ]
    ...
    for patch, cls in zip(bp["boxes"], class_order, strict=False):
        patch.set_facecolor(ICLABEL_COLORS[cls])
        patch.set_alpha(0.65)
    for pos, _cls, vals in zip(positions, class_order, data, strict=False):
        if vals:
    ...
    fig.suptitle(
        "A. Representative AMICA components by ICLabel class",
        x=0.02,
        ha="left",
        fontsize=10,
        fontweight="bold",
...
def plot_sensor_artifact(out_dir: Path, raw_all, groups: dict) -> None:
    ...
    noise = (
        zscore_rows(raw_all.get_data(picks=noise_chs)[:, ::decim])
        if noise_chs
        else np.empty((0, scalp.shape[1]))
    )
    emg = (
        zscore_rows(raw_all.get_data(picks=emg_chs)[:, ::decim])
        if emg_chs
        else np.empty((0, scalp.shape[1]))
    )
    imu = (
        zscore_rows(raw_all.get_data(picks=imu_chs)[:, ::decim])
        if imu_chs
        else np.empty((0, scalp.shape[1]))
    )
    ...
    all_vals = np.concatenate(
        [vals[np.isfinite(vals)] for vals, _ in corr_maps if np.isfinite(vals).any()]
    )
    vmax_shared = float(np.nanmax(all_vals)) if all_vals.size else 1.0
    vmax_shared = max(vmax_shared, 0.05)
    topo_im = None
    topo_axes = []
    for idx, (vals, title) in enumerate(corr_maps):
    ...
    fig.suptitle(
        "ds004505 artifact-reference characterization, sub-04 first 10 min",
        fontsize=10,
        fontweight="bold",
        y=0.995,
    )
...
def plot_condition_locked_rms(
    ...
    for ax, condition in zip(
        axes, ("cooperative", "competitive", "moving", "stationary"), strict=False
    ):
...
def plot_component_heatmap(
    out_dir: Path, raw, ica, result, labels: list[str], probs: np.ndarray
) -> None:
    ...
    hi = np.trapezoid(
        psds[:, (freqs >= 30) & (freqs <= 80)], freqs[(freqs >= 30) & (freqs <= 80)], axis=1
    )
    lo = np.trapezoid(
        psds[:, (freqs >= 1) & (freqs < 30)], freqs[(freqs >= 1) & (freqs < 30)], axis=1
    )
...
def plot_pairwise_mi_by_method(
    ...
    rows: list[dict] = []
    pairwise_values: dict[str, list[float]] = {"PCA-whitened input": baseline_stats["values"]}
    ...
    for x, val in zip(x_all, mean_vals, strict=False):
        if np.isfinite(val):
    ...
    for xpos, val in zip(x, reductions, strict=False):
        if np.isfinite(val):
...
def plot_condition_ersp(
    ...
    try:
        ...
        print(
            f"  ERSP: full-recording source extraction failed ({exc}); falling back to analysis window",
            flush=True,
        )
    ...
    last_mesh = None
    for ax, cond in zip(axes, cond_order, strict=False):
        if cond not in powers:
            ax.text(
                0.5, 0.5, f"no events: {cond}", ha="center", va="center", transform=ax.transAxes
            )
            ax.axis("off")
            continue
        mesh = ax.pcolormesh(
    ...
    summary = {
        ...
        "sampled_events_per_condition": {cond: int(arr.shape[0]) for cond, arr in epoched.items()},
    }
...
def plot_seed_stability(
    ...
    def _spatial_patterns(ica_obj) -> np.ndarray:
        ...
        Works with both MNE-native ICA and the py_amica.fit_ica wrapper
    ...
    ytick_labels = [f"IC{order[i]:02d}  {safe_label(labels[order[i]])}" for i in range(n_comp_ref)]
    ax_heat.set_yticklabels(ytick_labels, fontsize=6.5)
    for tick_label, ic in zip(ax_heat.get_yticklabels(), order, strict=False):
    ...
    ax_bar.set_title(
        f"B. Mean over {len(extra_seeds)} refits", loc="left", fontsize=8, fontweight="bold"
    )
    ...
    print(
        ...
        + ", ".join(f"{s}={rt:.1f}" for s, rt in zip(extra_seeds, runtimes, strict=False)),
        flush=True,
    )
...
def plot_quality_matrix(
    ...
    for ax, (title, values, cmap_name) in zip(axes, metric_specs, strict=False):
    ...
    for tick_label, ic in zip(axes[0].get_yticklabels(), order, strict=False):
        tick_label.set_color(ICLABEL_COLORS.get(labels[ic], "#444444"))
...
def plot_per_ic_properties(
    ...
    used: set[int] = set()
    for cls in classes:
        candidates = [(i, probs[i]) for i in range(n_comp) if labels[i] == cls and i not in used]
        if candidates:
            ...
            used.add(top)
    if not chosen:
        fallback = sorted(range(n_comp), key=lambda i: -np.nan_to_num(probs[i], nan=-1.0))
        chosen = fallback[:4]
