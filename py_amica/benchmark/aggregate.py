...
CLI:
  python -m py_amica.benchmark.aggregate \\
      --results-dir results/v3_pilot_2000 \\
      --output-dir  results/v3_pilot_2000

Python:
  from py_amica.benchmark.aggregate import discover_runs, benchmark_row
"""
...
def component_rows(run: RunPayload):
    ...
    rho_per_ic = (
        dipolarity.get("rho_per_ic")
        if isinstance(dipolarity, dict) and "error" not in dipolarity
        else None
    )
        ...
        rows.append(
            {
                ...
                "dipole_x": float(dipole_x_arr[i])
                if dipole_x_arr and i < len(dipole_x_arr) and dipole_x_arr[i] is not None
                else None,
                "dipole_y": float(dipole_y_arr[i])
                if dipole_y_arr and i < len(dipole_y_arr) and dipole_y_arr[i] is not None
                else None,
                "dipole_z": float(dipole_z_arr[i])
                if dipole_z_arr and i < len(dipole_z_arr) and dipole_z_arr[i] is not None
                else None,
                "iclabel_brain": float(prob_i)
                if (label_i == "brain" and prob_i is not None)
                else None,
                "iclabel_muscle": float(prob_i)
                if (label_i in ("muscle", "muscle artifact") and prob_i is not None)
                else None,
                "iclabel_eye": float(prob_i)
                if (label_i in ("eye", "eye blink") and prob_i is not None)
                else None,
                "kurtosis": float(kurt_vals[i])
                if (kurt_vals and i < len(kurt_vals) and kurt_vals[i] is not None)
                else None,
        ...
        rows.append(
            {
                ...
                "step_size": None,
                "gradient_norm": None,
                "elapsed_s": float(iter_times[i])
                if i < len(iter_times) and iter_times[i] is not None
                else None,
            }
        )
    return rows
...
def summary_table(results_dir, subject=None, *, prefer_fixed=True):
    ...
    if prefer_fixed:
        fixed_stems = {
            p.stem.removesuffix("_fixed") for p in json_paths if p.stem.endswith("_fixed")
        }
        json_paths = [
            p for p in json_paths if not (p.stem in fixed_stems and not p.stem.endswith("_fixed"))
        ]
    rows = []
    for jp in json_paths:
        ...
        method_label = METHOD_DISPLAY.get(
            f"{block.get('backend', method_key)}_{block.get('device', 'cpu')}", method_key
        )
        rows.append(
            {
                "method": method_label,
                ...
                "mir_kbits_s": cm.get("kbits_per_sec", float("nan"))
                if "error" not in cm
                else float("nan"),
                "log2_abs_det_W": cm.get("log2_abs_det_W", float("nan"))
                if "error" not in cm
                else float("nan"),
                "remnant_pmi_percent": pmi.get("remnant_PMI_percent", float("nan"))
                if "error" not in pmi
                else float("nan"),
    ...
    for dur_dir in sorted(results_root.glob("dur_*")):
        ...
        for json_path in sorted(dur_dir.glob(f"benchmark_sub-*{suffix}")):
            ...
            row = {
                ...
                "n_iter": int(block.get("n_iter", 0) or 0),
                "runtime_s": float(block.get("runtime_s", float("nan"))),
                "mir_bits_per_sample": cm.get("bits_per_sample", float("nan"))
                if "error" not in cm
                else float("nan"),
                "mir_kbits_s": cm.get("kbits_per_sec", float("nan"))
                if "error" not in cm
