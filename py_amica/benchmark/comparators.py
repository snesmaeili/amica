...
def load_runner(script_path: Path | None = None):
    ...
    Defaults to the importable ``py_amica.benchmark.runner``. For back-compat
...
def resolve_fit_params(
    ...
    *,
    ...
    fp = dict(
        fit_params_override
        if fit_params_override is not None
        else DEFAULT_FIT_PARAMS.get(method, {})
    )
...
def fit_all_on_raw(
    ...
    *,
    ...
    for method in methods:
        ...
        ica.save(
            json_path.with_name(json_path.stem + "_ica.fif"), overwrite=True, verbose="WARNING"
        )
        written[method] = doc
...
def run_tolerance_sweep(
    ...
    *,
    ...
    runner_module if runner_module is not None else load_runner()
    sweep_rows = []
    for tol in tols:
...
def main() -> None:
    ...
    parser.add_argument(
        "--input-level", type=str, default="bids", choices=["auto", "bids", "merged"]
    )
    parser.add_argument(
        "--runner-path",
        type=str,
        ...
        help=(
            ...
            "(legacy CLI shim). Default: use py_amica.benchmark.runner."
        ),
    )
    ...
    for method in methods:
        out_path = out_dir / comparator_output_filename(args.subject, method, DEFAULT_HP_FREQ)
