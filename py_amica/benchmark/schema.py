...
def kappa_table(
    bench_df, *, target_minimum: int = KAPPA_TARGET_MINIMUM, target_paper: int = KAPPA_TARGET_PAPER
):
    ...
    for subject, sub_df in bench_df.groupby("subject"):
        ...
        k_eff = (
            float(k_eff_series.dropna().iloc[0])
            if k_eff_series is not None and not k_eff_series.dropna().empty
            else None
        )
