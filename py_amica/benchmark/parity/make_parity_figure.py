"""Build the Fortran AMICA 1.7 <-> pyamica parity figure + summary table.
...
def main():
    ...
    ap.add_argument(
        "--out", default="fig_fortran_parity", help="output stem (writes .pdf, .png, .tex)"
    )
    args = ap.parse_args()
    ...
    # --- Panel A: LL trajectory overlay (Fortran solid, pyamica dashed) ---
    for (label, d), c in zip(runs, colors, strict=False):
        ...
        axA.plot(xp, lp, "--", color=c, lw=1.2, label=f"{label} — pyamica")
    ...
    y = np.arange(n_m)
    h = 0.8 / max(n_r, 1)
    for j, ((label, d), c) in enumerate(zip(runs, colors, strict=False)):
    ...
    with tex.open("w", newline="\n", encoding="utf-8") as fh:
        ...
        fh.write(
            " & ".join(
                h.replace("%", "\\%").replace("|", "$|$").replace("Δ", "$\\Delta$") for h in header
            )
            + " \\\\\n\\hline\n"
        )
        for r in rows:
