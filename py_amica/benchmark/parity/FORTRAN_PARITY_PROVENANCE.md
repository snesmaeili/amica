...
Validated Fortran **AMICA 1.7** reference binary for pyamica numerical parity.
...
pyamica reimplements. Source = `pyamica-benchmark/fortran/amica17_patched.f90`
...
1. **OMP reduction guards** (`amica17_patched.f90`) — upstream reduces Newton/rho
   thread-local arrays unconditionally; the patched source wraps each in its `if`
   (only matters when `do_newton=.false.`).
1. **`comp_list` init under `fix_init`** (`amica17_patched.f90`, fix_init branch ~L813) —
...
**Conclusion:** pyamica reproduces AMICA 1.7 to optimizer precision.
...
loglik) and compared to pyamica's `sample_mask_`. From `results/parity_reject_synth6.json`:
...
pyamica's `invsigmax` clamp + guarded steps stay finite). The rejected SET is still
...
pyamica compare. It imports `py_amica.benchmark.parity.run_fortran_parity`
(harness lives in the pyamica repo). Submit on fir:
