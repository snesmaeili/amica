"""Cross-implementation parity: reference Fortran AMICA 1.7 vs pyamica.
...
def make_ds004505_reduced(
    subject_id=1, k_pca=64, sfreq=250.0, n_samples_cap=None, input_level="bids"
):
    ...
    """
    from py_amica.benchmark.runner import load_data, preprocess
...
def cmd_prep(args):
    ...
    np.save(wd / "X.npy", X)  # for pyamica in the compare phase
    ...
    reject_kw = (
        dict(
            ...
            numrej=args.numrej,
            rejsig=args.rejsig,
            rejstart=args.rejstart,
            rejint=args.rejint,
            write_LLt=1,
        )
        if getattr(args, "reject", False)
        else dict(write_LLt=0)
    )
    fio.write_param(
        wd / "amica.param",
        ...
        fix_init=1,
        use_min_dll=0,
        use_grad_norm=0,  # run full max_iter (no early stop) for matched-iter parity
        **reject_kw,
        **hp,
    )
    meta = dict(
        ...
        do_newton=int(bool(args.do_newton)),
        seed=int(args.seed),
        dataset=args.dataset,
        reject=bool(getattr(args, "reject", False)),
        numrej=int(args.numrej),
        rejsig=float(args.rejsig),
        rejstart=int(args.rejstart),
        rejint=int(args.rejint),
        spike_idx=([int(i) for i in spike_idx] if spike_idx is not None else []),
    )
    (wd / "meta.json").write_text(json.dumps(meta, indent=2), newline="\n")
    print(
        f"[prep] wrote {wd / 'data' / 'data.fdt'} ({X.shape}) + amica.param; m={args.m} "
...
def cmd_compare(args):
    ...
    X = (
        np.fromfile(wd / "data" / "data.fdt", dtype="<f4")
        .reshape((nc, meta["n_samples"]), order="F")
        .astype(np.float64)
    )
    ...
    # pyamica's fix_init reproduces AMICA 1.7's deterministic init exactly
    ...
    from py_amica.solver import Amica
    ...
    if meta.get("reject"):
        ...
        reject_cfg = dict(
            do_reject=True,
            rejsig=float(meta["rejsig"]),
            rejstart=max(1, int(meta["rejstart"]) - 1),
            rejint=int(meta["rejint"]),
            numrej=int(meta["numrej"]),
        )
    cfg = AmicaConfig(
        num_models=1,
        num_mix_comps=m,
        max_iter=meta["max_iter"],
        do_newton=bool(meta["do_newton"]),
        newt_start=hp["newt_start"],
        newt_ramp=hp["newt_ramp"],
        newtrate=hp["newtrate"],
        lrate=hp["lrate"],
        lratefact=hp["lratefact"],
        rholrate=hp["rholrate"],
        rholratefact=hp["rholratefact"],
        rho0=hp["rho0"],
        minrho=hp["minrho"],
        maxrho=hp["maxrho"],
        minlrate=1e-8,
    ...
    ll_traj_max_abs_diff = (
        float(np.max(np.abs(ll_f_traj[:n_common] - ll_py[:n_common]))) if n_common else float("nan")
    )
    ...
    if meta.get("reject"):
        ...
        _, ll_total = fio.read_fortran_llt(wd / "out", n_samples=meta["n_samples"], num_models=1)
        fort_rej = ll_total == 0.0
        py_rej = (
            ~np.asarray(res.sample_mask_)
            if res.sample_mask_ is not None
        ...
        out.update(
            ...
            rejected_intersection=inter,
            rejected_union=union,
    ...
    if fortran_ok:
        ...
        out.update(
            fortran_final_ll=ll_f_final,
            python_final_ll=ll_p_final,
            abs_ll_delta=abs(ll_p_final - ll_f_final),
            W_matched_abs_r_mean=mean_r,
            W_matched_abs_r_min=float(per_r.min()),
            W_aligned_frobenius_rel=fro_rel,
            matched_source_abs_r_mean=src_r,
            n_common_iters=n_common,
            ll_traj_max_abs_diff=ll_traj_max_abs_diff,
...
def main():
    ap = argparse.ArgumentParser(description="Fortran AMICA 1.7 vs pyamica parity")
    ...
    pp.add_argument(
        "--n-samples-cap", type=int, default=0, help="cap n_samples for real data (0 = use all)"
    )
    pp.add_argument(
        "--container-data",
        default="/data",
    ...
    pp.add_argument(
        "--container-out", default="/out", help="out dir path as seen by the Fortran binary"
    )
    ...
    pp.add_argument(
        "--spike-frac", type=float, default=0.03, help="fraction of samples to spike (reject mode)"
    )
    pp.add_argument(
        "--spike-amp", type=float, default=40.0, help="spike amplitude multiplier (reject mode)"
    )
