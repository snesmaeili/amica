    ...
    *,
    ...
    if zscore_rows:
        ...
        if W_square is None:
            log2_abs_det_W = 0.0
            det_note = (
                "W not provided; MIR omits the log2|det W| term (acts like entropy_separation)."
            )
        else:
    ...
    h_input = float(
        sum(
            entropy_histogram(X_sub[i], n_bins=n_bins, clip_sd=clip_sd)
            for i in range(X_sub.shape[0])
        )
    )
    h_sources = float(
        sum(
            entropy_histogram(Y_sub[i], n_bins=n_bins, clip_sd=clip_sd)
