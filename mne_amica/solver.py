...
class Amica:
    ...
    def fit(
        ...
        print(f"AMICA: Fitting {n_channels} channels x {n_samples} samples using {self.config.dtype}")
        ...
        # ========== Preprocessing ==========
        print("AMICA: Preprocessing (mean removal, sphering)...")
        ...
        print(f"AMICA: Using {n_components} components")
        ...
        for iteration in range(self.config.max_iter):
            ...
            if not is_good_val:
                lrate *= 0.5
                if iteration % 10 == 0:
                    print(f"  Iter {iteration}: NaN/Inf detected, reducing lrate to {lrate:.2e}")
            ...
            if iteration > 0 and dll < 0:
                ...
                if iteration % 10 == 0:
                     print(f"  Iter {iteration}: LL decreased, lrate adjusted to {lrate:.2e}")
            ...
            if iteration > 0 and self.config.use_min_dll:
                if dll < self.config.min_dll:
                    ...
                    if numincs > self.config.max_incs:
                        print(f"AMICA: Converged at iteration {iteration} (dll < min_dll)")
                        converged = True
                        iteration_times.append(time.perf_counter() - iter_start)
                        elapsed_times.append(time.perf_counter() - start_time)
                        break
            ...
            if lrate < self.config.minlrate:
                print(f"AMICA: Converged at iteration {iteration} (lrate < minlrate)")
                converged = True
                iteration_times.append(time.perf_counter() - iter_start)
                elapsed_times.append(time.perf_counter() - start_time)
                break
            ...
            # Progress output
            if iteration % 10 == 0:
                print(f"  Iter {iteration:4d}: LL = {ll_val:.6f}, lrate = {lrate:.2e}")
            ...
            if self.config.outdir is not None and self.config.writestep > 0 and (iteration + 1) % self.config.writestep == 0:
                ...
                print(f"  Saved checkpoint to {self.config.outdir}")
        ...
        if not converged:
            print(f"AMICA: Reached max_iter ({self.config.max_iter})")
    ...
    def save(self, outdir: Union[str, Path]) -> None:
        ...
        print(f"AMICA: Saved model to {outdir}")
