...
def test_M1_step_matches_fused(iteration, newt_start):
    ...
    # Snapshot the multimodel inputs BEFORE the fused step: _amica_step_fused has
    # donate_argnums on (W, A, c, alpha, mu, beta, rho) and deletes those buffers,
    # so the [None]-expanded copies must be materialised first (test-only; the real
    # fit loop always consumes the returned arrays).
    mm_in = (W[None], A[None], c[None], alpha[None], mu[None], beta[None], rho[None])
    out_s = _amica_step_fused(W, A, c, alpha, mu, beta, rho, gm, *args_common)
    out_m = _amica_step_multimodel(*mm_in, gm, *args_common)
