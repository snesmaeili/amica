"""Tests for amica_python.updates module."""
from __future__ import annotations

import numpy as np
import pytest

from amica_python import updates
...
def test_update_mu_beta_rho():
    ...
    from amica_python.pdf import compute_responsibilities
    ...
    rho_new = updates.update_rho_gradient(y_i, resp, mu_i, beta_i, rho_i, rholrate=0.1, minrho=1.0, maxrho=2.0)
...
def test_natural_gradient_and_newton_correction():
    ...
    Wtmp, posdef = updates.apply_full_newton_correction(
        jnp.asarray(dW), sigma2, kappa, lam
    )
...
def test_update_all_pdf_params():
    ...
    a_new, m_new, b_new, r_new = updates.update_all_pdf_params(
        jnp.asarray(y), jnp.asarray(alpha), jnp.asarray(mu),
        jnp.asarray(beta), jnp.asarray(rho), config
    )
...
def test_apply_updates_from_stats():
    ...
    b_new = updates.apply_beta_update_from_stats(
        beta_cur, beta_numer, mu_d1, mu_d2, rho, 1e-5, 1e5
    )
    assert np.asarray(b_new).shape == (n_mix, n_comp)

    rho_numer = jnp.asarray(rng.randn(n_mix, n_comp))
    rho_denom = jnp.asarray(rng.rand(n_mix, n_comp) + 0.1)
    r_new = updates.apply_rho_update_from_stats(
        rho, rho_numer, rho_denom, 0.1, 1.0, 2.0
    )
    assert np.asarray(r_new).shape == (n_mix, n_comp)
    ...
    s2, k, l = updates.compute_newton_terms_from_stats(
    ...
    assert np.asarray(l).shape == (n_comp,)
